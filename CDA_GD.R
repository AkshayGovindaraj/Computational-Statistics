...
library(nnet)
multinomModel <- multinom(V1 ~ ., data=train) # multinom Model
...
for(i in c(1:198)){
  ...
  gradbet2[1] = gradbet2[1] + exp(-xibet2)/gamma
  gradbet3[1] = gradbet3[1] + exp(-xibet3)/gamma
  gradbet4[1] = gradbet4[1] + exp(-xibet4)/gamma
  ...
  for(j in c(1:27)){
    gradbet2[j+1] = gradbet2[j+1] + train[i,j+1]*exp(-xibet2)/gamma
    gradbet3[j+1] = gradbet3[j+1] + train[i,j+1]*exp(-xibet3)/gamma
    gradbet4[j+1] = gradbet4[j+1] + train[i,j+1]*exp(-xibet4)/gamma
